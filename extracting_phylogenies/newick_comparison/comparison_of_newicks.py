...
def get_newick(path):
    if not path:
        exit(f"[{get_time()}] Error in get_newick(): Path cannot be None.")
...
def get_taxa(newick):
    ...
    Taxa are words in the newick string after spaces, commas and opening parentheses
...
def get_clades_with_distances(newick):
    """
    Given a newick string returns a list with all clades.
    If the given newick has correct formatting the last element of the list should always be the newick itself.

    Args:
        newick (str): newick string

    Returns:
        dict: dict containing all clades of the given newick and their respective distance 
    """
    parentheses_stack = []
    parentheses_dict = dict()
    for i in range(len(newick)):
        # append the indices of the opening parentheses to the stack 
        if newick[i] == "(":
            parentheses_stack.append(i)
            continue
        if newick[i] == ")":
            # the last appended opening parentheses corresponds to the first closing one
            parentheses_dict[parentheses_stack.pop()] = i
            continue
    print(parentheses_dict)
    # list of all clades
    clades_dict = dict()
    for open, close in parentheses_dict.items():
        # include the clades distance if there is one
        if close + 2 < len(newick) and newick[close + 1] == ":":
            # look for the first match after the closing parentheses
            trunc_newick = newick[close:]
            distance = re.search(r"(?<=\):)\d+\.\d+", trunc_newick).group()
            close += len(distance)
        else:
            # let outermost clade include the ; 
            # => outermost clade and last clade in the list should always be the newick itself
            close += 1
        # +  for the index of the closing parentheses, +1 for the ':'
        clades_dict[newick[open:(close+2)]] = distance
    return clades_dict

def get_leaf_distances(newick):
    leaf_distance_dict = dict()
    for taxon in get_taxa(newick):
        leaf_distance_dict[taxon] = re.search(f"(?<={taxon}:)\d+\.\d+", newick).group()
    return leaf_distance_dict

#TODO: implement sort newick function that canonicalizes a newick 
#TODO: implement counter for the amount of times a generated newick wasnt a in correct format
#TODO: penalize a broken format fully i.e. max rf distance?
def get_filename(path):
    ...
    if filename_match:
        ...
        exit(f"[{get_time()}] Error in get_filename(): Path doesn't contain a file name.")
...
def hamming_distance(str1, str2):
    ...
    if not len(str1) == len(str2):
        exit(f"[{get_time()}] Error in hamming_distance(): Strings have to be of equal length.")
    hamming_distance = 0
...
def get_taxon_pairs_greedy(original_taxa, generated_taxa):
    ...
    if len(original_taxa) < len(generated_taxa):
        print(f"[{get_time()}] Warning in get_taxon_pairs_greedily: More taxa in generated Newick than in original one.")
    elif len(original_taxa) > len(generated_taxa):
        print(f"[{get_time()}] Warning in get_taxon_pairs_greedily: More taxa in original Newick than in generated one.")
    pair_dict = dict()
...
def compare_taxa(original, generated):
    ...
    if not ptan.is_newick(original, 1): 
        exit(f"[{get_time()}] Error in compare_taxa(): Newick 1 is not formatted correctly.")
    elif not ptan.is_newick(generated, 1):
        exit(f"[{get_time()}] Error in compare_taxa(): Newick 2 is not formatted correctly.")
    elif not get_taxa(original) or not get_taxa(generated):
        exit(f"[{get_time()}] Error in compare_taxa(): Lists of taxa can't be empty.")
...
def compare_topology(original, generated):
    ...
    """
    if not ptan.is_newick(original, 1): 
        exit(f"[{get_time()}] Error in compare_topology(): Original Newick is not formatted correctly.")
    elif not ptan.is_newick(generated, 1):
        exit(f"[{get_time()}] Error in compare_topology(): Generated Newick is not formatted correctly.")
    ...
    # compare edges (includes trivial edges: (<taxon>,))
...
# for those common edges how different from each other are the distances of the original compared to the generated?
def compare_distances(original, generated):
    ...
    for original_taxon, generated_taxon in taxa_pairs.items():
        generated.replace(generated_taxon, original_taxon)
    # get distances of leaves of both newicks and compare them
    orig_leaves_dict = get_leaf_distances(original) 
    gen_leaves_dict = get_leaf_distances(generated)
         
    # TODO: get distances of clades of both newicks and compare them
    orig_clades_dict = get_clades_with_distances(original)
    gen_clades_dict = get_clades_with_distances(generated)
    return comparison_dict
def write_tsv_header(outfile_path):
    if os.path.exists(outfile_path):
        exit(f"[{get_time()}] Error in write_tsv_header: File already exists.")
    ...
    print(f"[{get_time()}] write_tsv_header: File with header was created.")

def write_tsv_entry(outfile_path, path_original_newick, path_generated_newick):
    if not os.path.exists(outfile_path):
        exit(f"[{get_time()}] Error in write_tsv_entry: File hasn't been initialized.")
    ...
    print(f"[{get_time()}] write_tsv_entry: Entry was written to file.")
...
def main():
    ...
    if __name__ == '__main__': 
        argument_parser = argparse.ArgumentParser(
            ...
            Compares two given Newicks by their taxa, distances and general topology. Results are saved to .tsv.""",
        ...
        if not outfile_path or not path_generated_newick or not path_original_newick:
            exit(f"[{get_time()}] Error in compare_newicks mode: When using '--mode compare_newicks' "
